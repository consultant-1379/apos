 ...
 *
 ----------------------------------------------------------------------*//**
 *
 * @file apos_ha_agent_reactorrunner.cpp
 *
 * @brief
...
int APOS_HA_ReactorRunner::svc() 
{
	...
	if (m_name.compare(MAIN_REACTOR) == 0 || m_name.compare(DRBDMON_REACTOR) == 0) {
...
//	if (!HA_AGENT_Global::instance()->shutdown_ordered()) {
//		HA_LG_ER("%s(u_long) Abnormal shutdown of APOS_HA_ReactorRunner", __func__);
//		exit(EXIT_FAILURE);
//	}
