...
#       properties
# Description:
#	A script for fetch the deployment properties in an openstack based data center.
#
##
# Usage:
#       call: properties.sh
...
PROPERTIES_ALLOWED_FETCH_METHOD="configdrive"
...
for FETCH_METHOD_ITEM in "$PROPERTIES_ALLOWED_FETCH_METHOD";do
  if [ -z $PROPERTIES_SUBCOMMAND ];then
    /opt/ap/apos/bin/gi/lib/fetch_method/$FETCH_METHOD_ITEM.sh | grep "PROPERTIES:" | awk -F'PROPERTIES:' '{print $2}'
    if [ $? -eq 0 ];then
      exit 0
    fi
  else
    for PROPERTIES_FOUND_ITEM in $(/opt/ap/apos/bin/gi/lib/fetch_method/$FETCH_METHOD_ITEM.sh | grep "PROPERTIES:" | awk -F'PROPERTIES:' '{print $2}');do
