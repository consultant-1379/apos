...
function install_files(){
	...
	for L in $LINES; do
		...
		if [ -L $SRC_DIR/$FILE ]; then
                        cp --no-dereference $SRC_DIR/$FILE $DEST_DIR/$FILE || abort "failure during the copy of the $FILE symbolic link"
	...
	local FOLDER_PERM=$(get_lines)
	for L in $FOLDER_PERM; do
		local PERM=$(echo $L|awk -F':' '{print $1}')
		local FILE=$(echo $L|awk -F':' '{print $2}')

		if [ -L $SRC_DIR/$FILE ]; then
			echo "No permission changes are required for softlinks"			
		elif [ -d $SRC_DIR/$FILE ]; then
			#echo "mkdir -p -m $PERM $DEST_DIR/$FILE"
