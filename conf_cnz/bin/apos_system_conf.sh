...
function create_structure(){
  ...
  apos_log "INFO: apos_systemconf --> Begin --apos blk"
  ...
  apos_log "INFO: apos_systemconf--apos blk --> THIS_IP=$THIS_IP"
  if [ -n "$THIS_IP" ]; then
    if ! echo "$IP_LIST" | grep -q "$THIS_IP"; then
      if ! /usr/bin/cluster config --create; then
        apos_abort 1 "Failed to create configuration structure --apos blk"
      fi
    fi
  fi
  apos_log "INFO: apos_systemconf --> End --apos blk"
}

#-------------------------------------------------------------------------------
function configure_local_macs() {
  echo 'current nic/mac setup --apos blk'
  show_current_nic_setup
  set_local_macs_in_cluster_conf local
  echo 'local macs correctly setup --apos blk'
}

#-------------------------------------------------------------------------------
function configure_local_nics() {
  echo 'configure nics in cluster.conf'
  add_custom_interfaces_in_cluster_conf local
  echo 'local nics correctly setup'
}
...
#                                             |_|    |_| |_|   |_| |_| |_|  \__|
#
echo "In apos_system_conf.sh --apos blk"
case $1 in
  early)
