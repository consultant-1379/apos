...
##
# Changelog:
# - Mon Aug 07 2023 - Surya Mahit (zsurjon)
#   Fix for TR IA39761
# - Thu June 22 2023 - Anjali M(XANJALI)
#   Updated isvBSC()
# - Thu Apr 20 2023 - Srimukhi Nagalla (ZSGRAIN)
#   modified isvBSC function to extend support for VMware 
# - Wed Aug 03 2022 - Amrutha Padi (zpdxmrt)
#   Fix for TR HZ59046 
# - Fri Apr 01 2022 - Pratap Reddy (xpraupp)
#   modified isvBSC function
...
function apos_check_and_exlocall() {
  ...
  apos_log 'before lockfile'
  lockfile -${sleeptime} -r ${retries} ${fl_fold}/${fl_file} &>/dev/null
  apos_log 'after lockfile'
  if [ $? -ne $TRUE ]; then
    rm -f ${fl_fold}/${fl_file}
  ...
  apos_log 'before executing script after acquiring lockfile'
  if [ -x $1/$2 ]; then
    ./$2
    if [ $? -ne $TRUE ]; then
      rm -f ${fl_fold}/${fl_file}
...
function umount_cdrom() {
  ...
  local UMOUNT_CMD="/usr/bin/timeout --signal=INT --kill-after=$SIGKILL_TMOUT $SIGINT_TMOUT /bin/umount $MOUNT_FOLDER"
  # executes the umount instruction
  $UMOUNT_CMD

  local RETURN_CODE=$?
  while [[ $RETURN_CODE -ne 0 && $attempts -lt $max_attempts ]]; do
    apos_log "unable to unmount $mount_point... retrying"
    attempts=$(( $attempts + 1 ))
  ...
  if /usr/bin/mountpoint -q $MOUNT_FOLDER; then
    apos_log "unable to unmount \"$MOUNT_FOLDER\" after $max_attempts attempts!"
  else
    apos_log "config_drive successfully unmounted"
  fi
...
function mount_config_drive() {
  ...
  for label in $(find "/dev/disk/by-label" -maxdepth 1 -type l ); do
    if [[ "$(/usr/bin/basename $label)" == 'config-2' || "$(/usr/bin/basename $label)" == 'OVF\x20ENV' ]]; then
      if [ -L $label ]; then
  ...
  if [ -n "$config_drive" ]; then
    ...
    if /usr/bin/mountpoint -q "$config_dir";then
      apos_log "Folder $config_dir already mounted."
      umount_cdrom $config_dir
    fi

    # mount the config_drive with read only option
    if ! /bin/mount $OPTS "$config_drive" "$config_dir" 2>/dev/null; then
      apos_log "Failed to mount config drive"
      return $FALSE
    else
      apos_log "config_drive mounted successfully on $config_drive"
    fi
  else
    ## Handle it in when vBSC supports VMware also 
    ## mount_cdrom_by_content
    apos_log "config_drive label not found, it is not Openstack env"
    return $FALSE
  fi
...
function isvBSC(){
  ...
  local OVF_ENV_FILE="$config_dir/ovf-env.xml"
  local CMD_HWTYPE='/opt/ap/apos/conf/apos_hwtype.sh'
  local HW_TYPE=$( $CMD_HWTYPE)
  local ENV_TYPE=$( $CMD_HWTYPE --verbose | grep "system-manufacturer" | awk -F"=" '{print $2}' |\
        sed -e 's@^[[:space:]]*@@g' -e 's@^"@@g' -e 's@"$@@g' )

  if [ "$HW_TYPE" == 'VM' ]; then
  
    apos_log 'HW_TYPE Found VM'
 
    # mount config drive to fetch apt_type 
    if mount_config_drive; then

      if [[ "$ENV_TYPE" =~ .*vmware.* ]]; then
        if [ -z "$OVF_ENV_FILE" ]; then
          apos_abort "ERROR: OVF_ENV file not found"
        fi

        property_value=$(cat $OVF_ENV_FILE  | grep -w 'apt_type' | awk -F'oe:value=' '{print $2}' |awk '{print $1;exit}' | tr -d '"/>')

      elif [[ "$ENV_TYPE" =~ .*openstack.* ]]; then
        CONFIGDRIVE_FILE_USERDATA="$(find "$config_dir/openstack/latest" -maxdepth 1 -type f -name "user_data")"
        if [ -z "$CONFIGDRIVE_FILE_USERDATA" ]; then
          apos_abort "ERROR: user_data file not found"
        fi

        PARSING_OUTPUT=$($OPENSTACK_USERDATA_PARSE_SCRIPT $CONFIGDRIVE_FILE_USERDATA | grep -w 'apt_type')
        if [ -n "$PARSING_OUTPUT" ]; then
          property_value="$(echo "$PARSING_OUTPUT" | awk -F'=' '{print $2}')"
        fi
      else
        apos_abort "Unsupported virtual environment!! Supported virtual environments are: Openstack,VMware"
      fi

      if [ -n "$property_value" ]; then
        [ "$property_value" == "BSC" ] && RESULT=$TRUE
      else
        apos_log "isvBSC(): apt_type property_value found NULL!!"
        return $RESULT
      fi

      # unmount the config drive after fetching apt_type
      umount_cdrom $config_dir
    else
      apos_abort "config_drive not found"
    fi
  else
    apos_log "HW_TYPE Found non-virtual"
  fi
...
function isSCX(){
  local HW_TYPE=$(/opt/ap/apos/conf/apos_hwtype.sh)
  local SHELF_ARCH=$(get_shelf_architecture)
  [[ "$SHELF_ARCH" == "SCX" && "$HW_TYPE" == 'GEP7' ]] && return $TRUE
  return $FALSE
}
