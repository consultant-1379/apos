...
function set_local_macs_in_cluster_conf(){
  ...
  if [ ! -w "$cluster_file" ]; then
    ...
    message="setting local macs in $cluster_file"
    echo "$message"
    apos_log "$message"
  fi
  ...
  while read net_entry; do
    ...
    node_id=$(get_node_id)
    echo "setting mac ${mac} to nic ${nic}... --apos blk"
    if [ $mutex -eq $TRUE ]; then
      lock
    fi
    usleep 10000
    echo "after executing sleep of 10ms"
    ...
    if [ $return_code -ne $TRUE ]; then
      echo "executing trap -EXIT in case the return code is not true --apos blk"
      trap - EXIT
      message="failure while executing sed in-place on the file --apos blk \"$cluster_file\" ($(<${tmp_error}))"
      echo "ABORT: $message" >&2
      apos_abort "$message"
    fi
    apos_log 'done'
